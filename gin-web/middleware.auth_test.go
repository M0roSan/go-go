...
// Test ensureLoggedIn middleware when the user is not logged in
func TestEnsureLoggedInUnauthenticated(t *testing.T) {
	r := getRouter(false)

	r.GET("/", setLoggedIn(false), ensureLoggedIn(), func(c *gin.Context) {
		// since we aren't logged in, this handler shouldn't be executed
		// if it is, then the ensureLoggedIn isn't working as exepcted
		t.Fail()
	})
...
// Test ensureLoggedIn middleware when the user is logged in
func TestEnsureLoggedInauthenticated(t *testing.T) {
	...
	// Use the helper method to execute process the request and test the HTTP status code
...
func TestSetUserStatusAuthenticated(t *testing.T) {
	...
	r.GET("/", setUserStatus(), func(c *gin.Context) {
		// as the token cookie was set, "is_logged_in" should have been set to true
...
func TestSetUserStatusUnauthenticated(t *testing.T) {
	r := getRouter(false)
	r.GET("/", setUserStatus(), func(c *gin.Context) {
		// as the token cookie was set, "is_logged_in" should have been set to false
		loggedInInterface, exists := c.Get("is_logged_in")
		if exists && loggedInInterface.(bool) {
			t.Fail()
		}
...
// This is a middleware that will set the value of "is_logged_in" to trye or false
